...
x = [2.0, 3.0, -1.0]    
n = MLP(3, [4, 4, 1]) #(dimensione input, [layer di 4, layer di 4, n° output])
#print(n(x)) #da qui si ottengono valori tutti diversi perchè abbiamo messo randomici sia w che b
...
#abbiamo 4 possibili input della rete neurale e ys sono i target desiderati
ys = [1.0, -1.0, -1.0, 1.0]

ypred = [n(x) for x in xs]
print(ypred) #i risultati ovviamente non coincidono con quelli di ys, perciò bisogna calcolare una funzione di perdita per misurare la distanza
#in questo momento che osono risultati molto lontani non è molto performante ovviamente

loss = sum((yout.data - ygt)**2 for ygt, yout in zip(ys, ypred)) #errore quadratico medio visto e rivisto in statistica. Itera quindi coppie di tuple
#yout.data e non yout perchè mi dice che non supporta operazioni tra int e Value, credo per la funzione sum. In ogni caso con Yout Data funziona
print(loss)

#loss.backward() non so perchè ma mi dice che non ha la funzione backward, credo per i problemi dei fiels precedenti anche se non capisco
n.layers[0].neurons[0].w[0]

for p in n.parameters():
    p.data += -0.01 * p.grad #vado ad aumentare il dato per una piccolissima parte del gradiente
#in questo caso mettiamo -0.01 perché la derivata è negativa, quindi la funzione di perdita andrebbe ad aumentare con uno 0.01 positivo
#quindi bisogna iterare il processo